...
class TimerEventAlter {
  TimerEventAlter._() {
    _startMonitoring();
  }

  static final TimerEventAlter instance = TimerEventAlter._();
  ...
  ActivityStatus _lastActivityStatus = ActivityStatus.ACTIVE;
  ...
  void _startMonitoring() {
    keyboardMouseDetector.listenKeyMouseEvent.listen((event) {
      if ((event is MouseMoveEvent) == false) {
        ...
        if (_lastActivityStatus != ActivityStatus.ACTIVE) {
          _statusEvent?.call([
            TimerEventModel(status: _lastActivityStatus, startTime: _lastUpdatedActivityTime, endTime: _activityStartTime),
            TimerEventModel(status: ActivityStatus.ACTIVE, startTime: _activityStartTime, endTime: _lastActivityTime),
          ]);
          _lastActivityStatus = ActivityStatus.ACTIVE;
      ...
      if (_privateToggled == false) {
        ...
        if (timerDifference > zeroDuration) {
          if (difference < _idleDuration) {
            ...
            _lastActivityStatus = ActivityStatus.IDLE;
            _log(
            ...
            _lastActivityStatus = ActivityStatus.BREAK;
            _log(
...
final timerAlterEvent = TimerEventAlter.instance;
